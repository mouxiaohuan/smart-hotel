...
type PendingRefundIntent = { query: string; threadId: string };

const TOKEN_KEY = 'smart-hotel.member-token';
const PENDING_REFUND_KEY = 'smart-hotel.pending-refund-intent';
...
export default function Home() {
  ...
  const [threadId] = useState(() => typeof window === 'undefined' ? 'web-server' : sessionStorage.getItem('smart-hotel.thread-id') ?? `web-${crypto.randomUUID()}`);

  useEffect(() => {
    sessionStorage.setItem('smart-hotel.thread-id', threadId);
    fetch('/api/knowledge').then((res) => res.json()).then(setKnowledge);
    const pending = sessionStorage.getItem(PENDING_REFUND_KEY);
    if (pending && sessionStorage.getItem(TOKEN_KEY)) {
      sessionStorage.removeItem(PENDING_REFUND_KEY);
      const intent = JSON.parse(pending) as PendingRefundIntent;
      void ask(intent.query);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  function redirectToLogin(intent: PendingRefundIntent) {
    sessionStorage.setItem(PENDING_REFUND_KEY, JSON.stringify(intent));
  ...
  async function ask(value: string) {
    ...
    try {
      ...
      const response = await fetch('/api/ask/stream', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-thread-id': threadId, ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify({ query }) });
      ...
      while (true) { const chunk = await reader.read(); if (chunk.done) break; buffer += decoder.decode(chunk.value, { stream: true }); const events = buffer.split('\n\n'); buffer = events.pop() ?? ''; for (const event of events) { const line = event.split('\n').find((item) => item.startsWith('data: ')); if (!line) continue; const payload = JSON.parse(line.slice(6)) as { step?: string; code?: string } & Result; if (payload.code === 'AUTH_REQUIRED') authRequired = true; if (payload.step) setMessages((items) => [...items, { role: 'bot', text: `处理中：${payload.step}` }]); if (payload.answer) data = payload; } }
      if (authRequired) { redirectToLogin({ query, threadId }); return; }
